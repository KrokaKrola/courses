...
const DataFieldContext = createContext()
/*
*
    <Fragment>
...
* */
 ...
 export default function DateFields({
  ...
  const date = controlledValue || "March 1, 2019"
  return (
    <DataFieldContext.Provider value={{ date, onChange }}>
      {children.map((item,index) => {
        return index < children.length - 1 ? [item, separator] : [item]
      })}
    </DataFieldContext.Provider>
  )
}

// export function DataDateFields({data}) {
//   return <DateFields value={data.value} onChange={data.onChange}>
//     <MonthField/>
//     <DayField/>
//     <YearField start={data.start} end={data.end}/>
//   </DateFields>
// }

export function DayField(props) {
  const { date, onChange } = useContext(DataFieldContext)
